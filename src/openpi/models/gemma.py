...
class RMSNorm(nn.Module):
    ...
    def __call__(self, x, cond):
        ...
        if modulation.ndim == x.ndim - 1:
            # [b, 3d]: a single modulation shared by every token in the sequence.
            modulation = modulation[:, None, :]
        # otherwise [b, t, 3d]: one modulation per token (used by RTC's trained mode,
        # which drives the prefix tokens at a different flow-matching timestep). The
        # Dense above acts on the last axis either way, so the params are unchanged.
