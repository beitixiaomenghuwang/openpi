#!/usr/bin/env python3
"""
Environment wrapper for the Teleavatar v2 robot using end-effector representation.
"""
...
from examples.teleavatar_v2 import ros2_interface_endeffector
...
class TeleavatarEndEffectorEnvironment(_environment.Environment):
    ...
    def __init__(
        ...
        ros2_interface_endeffector._POLICY_TO_RTP_VIEW), matching the crops
    ...
    def _init_ros2(self):
        ...
        if not self._ros_interface.wait_for_initial_data(timeout=30.0):
            raise RuntimeError(
                ...
                "Please check that the RTP video stream and ROS2 topics are up:\n"
    ...
    def get_observation(self) -> dict:
        ...
        Returns:
            Dictionary with keys:
                - 'observation/state': 62-dim proprioceptive state
                  [positions(16), velocities(16), efforts(16),
                   left_ee_pose(7), right_ee_pose(7)]
                - 'observation/images/*': camera images in (H, W, C) format
        ...
        # Return with the exact keys expected by teleavatar_v2_policy_endeffector.py
        return {
    ...
    def apply_action(self, action: dict) -> None:
        ...
        Args:
            action: Dictionary containing 'actions' key with 16-dim action array
                    [left_ee_target_pose(7), left_gripper_effort(1),
